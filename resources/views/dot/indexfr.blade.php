...
<body>
    ...
    <div class="content">   
        ...
        @endif
        <form action="{{ route('dot.filterfr') }}" method="GET">
            <label for="start_date" id="strtdt">Date debut:</label>
            ...
            <a href="{{ route('dot.indexfr') }}" class="btn btn-primary">Réinitialiser</a>
        </form>

        <form action="{{ route('dot.indexfr') }}" method="GET"></form>
        <table  id="dotTable">
            <thead>
                <tr>
                    <th>ID</th>
            ...
            <tbody>
                @foreach($users as $user)
                    <tr>
                        ...
                        <td>{{ $user->Wilaya_user }}</td>
                        {{-- <td>{{ $user->Created_at }}</td> --}}
                        <td><a class="btn" href="{{ route('dot.editfr', $user->ID_user) }}">Modifier</a></td>
                            <td>
                            <form action="{{ route('dot.destroyfr', $user->ID_user) }}" method="POST" style="display:inline-block;">
                                @csrf
                                @method('DELETE')
                                <button  class="btn" type="submit" onclick="return confirm('Are you sure?')">Suppriemr</button>
                            </form>
                        </td>
                    </tr>
                @endforeach
